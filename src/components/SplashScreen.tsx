...
export default function SplashScreen({ onComplete }: SplashScreenProps) {
  ...
  useEffect(() => {
    // 1. Fade out the loading spinner slightly before the splash screen exit
    const spinnerTimer = setTimeout(() => {
      setLoadingVisible(false);
    }, 1700);

    // 2. Trigger the exit animation of the splash screen
    const exitTimer = setTimeout(() => {
      setIsExiting(true);
    }, 2100);

    // 3. Complete the splash screen phase
  ...
  return (
    <AnimatePresence>
      {!isExiting && (
        ...
        >
          {/* Subtle background ambient lights for premium feels (Threads/Discord style) */}
          ...
          {/* Dummy element for flex spacing */}
          <div className="h-4" />
          ...
          {/* Modern gold loading circle at the bottom */}
